...
Rubylike.Array.new = function (size, val) {
	...
	switch (size.class()) {
		...
		case 'Undefined':
			break;
		default:
			throw new TypeError("`initialize': can't convert " + size.class() + " into Integer");
			break;
	}
	return ret;
};
Rubylike.Array.prototype.assoc = function (key) {
	for (var i = 0, len = this.length; i < len; i += 1) {
		var target = this[i];
		if (target.class() === 'Array' && ((target[0] == key) || (typeof target[0] === 'object' && target[0].eql(key)))) {
			return target;
		}
	}
	return null;
 ...
 *   val : setting value
 *   start : start index
 *   length : setting count (if length = nil then to array length)
 *   block : arguments is self index
 */
